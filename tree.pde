...
void draw() {
  ...
  TreeNode<PlantNode> getSet() {
    TreeNode<PlantNode> root = new TreeNode<PlantNode>(new PlantNode(width/2, 0, PI/2));             
      ...
      TreeNode<PlantNode> node300 = spli(node30, 0, PI/4.f);      
      TreeNode<PlantNode> node301 = spli(node30, 1, PI/4.f);
  ...
  TreeNode<PlantNode> spli(TreeNode<PlantNode> t, int i, float d) {
    float direction = t.data.direction;
    float x = t.getData().x;
    float y = t.getData().y;
